...
# Base de données des matières simulée (prix et densités réalistes)
MATERIAL_DATABASE: dict[str, dict] = {
    ...
    "cuivre": {"price_per_kg": 9.50, "density_kg_m3": 8960},
    "default": {"price_per_kg": 1.80, "density_kg_m3": 7850},
}


class QuotationService:
    """Moteur de calcul de devis pour pièces de tôlerie et d'usinage.

    Les coûts sont décomposés en 5 postes :
    - Matière (masse × prix/kg)
    - Découpe laser (temps fonction du périmètre + mise en place)
    - Pliage (temps par pli + setup presse plieuse)
    - Perçage (temps par trou)
    - Montage/finition (setup + composants à intégrer)
    Une marge commerciale est appliquée en fin de calcul.
    """
    ...
    def generate_quotation(self, request: QuotationRequest) -> QuotationResponse:
        ...
        taux_operateur = request.taux_horaire_operateur or self.settings.operator_hourly_rate

        mat_cost = self._calc_material(drawing, warnings)
        cut_cost = self._calc_cutting(drawing, taux_machine)
        bend_cost = self._calc_bending(drawing, taux_machine)
        drill_cost = self._calc_drilling(drawing, taux_machine)
        asm_cost = self._calc_assembly(drawing, taux_operateur)

        sous_total = (
            mat_cost.cout_total
            + cut_cost.cout_total
            + bend_cost.cout_total
            + drill_cost.cout_total
            + asm_cost.cout_total
        )
    ...
    def _calc_material(self, drawing: DrawingData, warnings: list) -> MaterialCost:
        ...
        if masse_kg == 0 and drawing.dimensions.surface_depliee_mm2 and drawing.epaisseur_mm:
            ...
            volume_m3 = surface_m2 * (drawing.epaisseur_mm / 1000)
            masse_kg = volume_m3 * params["density_kg_m3"]

        if masse_kg == 0:
            masse_kg = 0.1
            warnings.append("Masse non calculable depuis le plan — valeur minimale utilisée (100 g)")

        # Ajout de 15% de chute matière
    ...
    def _calc_cutting(self, drawing: DrawingData, taux: float) -> MachiningCost:
        ...
        vitesse_mm_min = 3000  # vitesse laser CO₂ typique sur acier 2 mm
        temps = perimetre / vitesse_mm_min + 1.5  # 1,5 min de mise en place
        return MachiningCost(
            operation="Découpe laser",
            temps_minutes=round(temps, 2),
            taux_horaire=taux,
            cout_total=round((temps / 60) * taux, 4),
        )
    ...
    def _calc_bending(self, drawing: DrawingData, taux: float) -> MachiningCost:
        ...
        temps = 5.0 + nb * 0.8  # 5 min setup + 0,8 min/pli
        if nb == 0:
            temps = 0
        return MachiningCost(
            operation="Pliage (presse plieuse)",
            temps_minutes=round(temps, 2),
            taux_horaire=taux,
            cout_total=round((temps / 60) * taux, 4),
            quantite=nb,
    ...
    def _calc_drilling(self, drawing: DrawingData, taux: float) -> MachiningCost:
        ...
        temps = nb * 0.5  # 0,5 min par perçage
        return MachiningCost(
            operation="Perçage / Poinçonnage",
            temps_minutes=round(temps, 2),
            taux_horaire=taux,
            cout_total=round((temps / 60) * taux, 4),
            quantite=nb,
    ...
    def _calc_assembly(self, drawing: DrawingData, taux: float) -> MachiningCost:
        nb_composants = sum(i.quantite for i in drawing.nomenclature) if drawing.nomenclature else 0
        temps = 10.0 + nb_composants * 2.5  # 10 min setup + 2,5 min/composant
        return MachiningCost(
            operation="Montage / Finition",
            temps_minutes=round(temps, 2),
            taux_horaire=taux,
            cout_total=round((temps / 60) * taux, 4),
            quantite=nb_composants,
        )
    ...
    def _validate(self, total_ht: float, drawing: DrawingData, warnings: list) -> None:
        ...
        if total_ht > 5000:
            warnings.append("Coût total élevé — vérifier les dimensions et la matière")
        if drawing.confidence_score < 0.5:
            warnings.append(
                f"Confiance d'extraction faible ({drawing.confidence_score:.0%}) "
                "— résultats à valider manuellement avant envoi"
            )
